...
This notebook runs an example analysis using ChromaSC. The notebook uses the "clone 14" timecourse data from an analysis of chemoresistance in ovarian cancer detailed [here](https://www.biorxiv.org/content/10.1101/231548v1).
...
```{r}
caov3.hmrks <- res$enrichment[[1]]
parent.hmrks <- res$enrichment[[2]]
s02.hmrks <- res$enrichment[[3]]
s03.hmrks <- res$enrichment[[4]]
step5.hmrks <- res$enrichment[[5]]
step10.hmrks <- res$enrichment[[6]]
step15E.hmrks <- res$enrichment[[7]]
step15L.hmrks <- res$enrichment[[8]]
```

### Function to plot combined gene set Lowess

```{r}
plot.ratio.lowess <- function(gs.name, f=0.2, ylim.min=NA, ylim.max=NA, leg.cex=2.0, main.cex=2.0, leg.loc="bottomleft") {
  par(mar=c(5,6,4,1)+.1)
  ...
  plot(lowess(x, caov3.hmrks[gs.name,], f=f), col=cs[1], type='l', lwd=4, ylim=c(ylim.min, ylim.max), 
       main=gs.name, cex.axis=2, cex.main=main.cex, xlab="Trajectory Bin",
  ...
  legend(leg.loc, legend=leg.names, col=cs, lty=1, bty="n", cex=leg.cex, lwd=4)
}
```

```{r, fig.asp=0.7}
plot.ratio.lowess("HALLMARK_G2M_CHECKPOINT", f=0.2, ylim.min=0.2, ylim.max=2.0, leg.cex=1.2, leg.loc="bottomright")
```


```{r, fig.asp=0.8}
plot.ratio.lowess("HALLMARK_OXIDATIVE_PHOSPHORYLATION", f=0.2, ylim.min=1.5, ylim.max=2.5, leg.cex=1.2,
                  main.cex=1.3)
```
